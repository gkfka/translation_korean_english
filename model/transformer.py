...
class koTokenEmbedding(nn.Module):
  def __init__(self, ):
    super(koTokenEmbedding, self).__init__()
    self.embedding = ElectraModel.from_pretrained("monologg/kocharelectra-base-discriminator")

  def forward(self, tokens: Tensor):
    with torch.no_grad():
      output = self.embedding(input_ids=tokens)
    return output[0]
...
class Transformer(nn.Module):
  def __init__(self, en_vocab_num, ko_d_model, en_d_model, max_seq_len, head_num, dropout, N):
    super(Transformer,self).__init__()
    self.ko_embedding = koTokenEmbedding()
    self.en_embedding = Embeddings(en_vocab_num, en_d_model)
    self.ko_positional_encoding = PositionalEncoding(max_seq_len,ko_d_model)
    self.en_positional_encoding = PositionalEncoding(max_seq_len,en_d_model)


    self.encoders = clones(Encoder(d_model=ko_d_model, head_num=head_num, dropout=dropout), N)
    self.decoders = clones(Decoder(d_model=en_d_model, head_num=head_num, dropout=dropout), N)

    self.generator = Generator(en_d_model, en_vocab_num)

  def forward(self, input, target, input_mask, target_mask, labels=None):
      x = self.ko_positional_encoding(self.ko_embedding(input))
      for encoder in self.encoders:
        x = encoder(x, input_mask)

      target = self.en_positional_encoding(self.en_embedding(target))
