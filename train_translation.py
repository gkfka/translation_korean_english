...
class TranslationTrainer():
  def __init__(self,
    ...
    self.device = device
    self.ntoken = tokenizer1.vocab_size
  ...
  # def translate(self, input, input_str ):
  #   encoder_input = input
  #   encoder_mask = (encoder_input != tokenizer2.pad_token_id).unsqueeze(0)
  ...
  #       print('\n'+f'ko: {tokenizer2.decode(input.squeeze().tolist(),skip_special_tokens=True)} en: {tokenizer1.decode(target.squeeze().tolist(),skip_special_tokens=True)}')
  #       break
  ...
  def train(self, epochs, train_dataset, eval_dataset, optimizer, scheduler):
    ...
    for epoch in range(start_epoch, epochs):
      ...
      # self.translate(input, input_str)
      print('-' * 89)
  ...
  def save(self, epoch, model, optimizer, losses, train_step):
    ...
    }, f'{self.checkpoint_path}/{self.model_name}{epoch}.pth')
...
if __name__ == '__main__':
  ...
  en_vocab_path = 'data/wiki-vocab.txt'
  ko_vocab_path = 'kocharelectra/vocab.txt'
  # data_path = f'{dir_path}/data/ko-en-spoken.csv'
  data_path = 'data/1_구어체(1).csv'
  checkpoint_path = 'checkpoints'

  # model setting
  model_name = 'transformer-translation-spoken145'
  en_vocab_num = 22000
  ko_vocab_num = 11568
  max_length = 100
  en_d_model = 768
  ...
  tokenizer1 = BertTokenizer(vocab_file=en_vocab_path, do_lower_case=False)
  tokenizer2 = KoCharElectraTokenizer(vocab_file=ko_vocab_path, do_lower_case=False)
  # hyper parameter
  epochs = 1000
  batch_size = 4
  padding_idx = tokenizer1.pad_token_id
  learning_rate = 0.5

  dataset = TranslationDataset(tokenizer1=tokenizer1, tokenizer2=tokenizer2,file_path=data_path, max_length=max_length)

  model = Transformer(en_vocab_num=en_vocab_num,
                      ko_d_model=ko_d_model,
                      en_d_model=en_d_model,
  ...
  trainer = TranslationTrainer(dataset, tokenizer1, tokenizer2, model, max_length, device, model_name, checkpoint_path, batch_size)
