...
class TranslationDataset(Dataset):
  def __init__(self, tokenizer1:BertTokenizer, tokenizer2 :KoCharElectraTokenizer,  file_path:str, max_length:int):
    pad_token_idx = tokenizer1.pad_token_id
    csv_datas = load_csv(file_path)
    self.docs = []
    # for line in csv_datas: # line[1] 한글, line[2] 영어
    for line in tqdm(csv_datas):
      input = tokenizer2.encode(line[1],max_length=max_length,truncation=True)
      rest = max_length - len(input)
      input = torch.tensor(input + [pad_token_idx]*rest)

      target = tokenizer1.encode(line[2], max_length=max_length, truncation=True)
      rest = max_length - len(target)
      target = torch.tensor(target+ [pad_token_idx] * rest)

      doc={
        'input_str': tokenizer2.convert_ids_to_tokens(input),
        'input':input,                                        # input
        'input_mask': (input != pad_token_idx).unsqueeze(-2),       # input_mask
        'target_str': tokenizer1.convert_ids_to_tokens(target),
